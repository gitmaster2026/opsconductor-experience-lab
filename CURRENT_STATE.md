...
## Session log — 2026-07-22 V1-DEMO-1 Founder Demo Package, Demo Reset, and Rehearsal Validation

Scope: launch-readiness and demo-operations only - a deterministic Demo Reset action, founder demo scripts/runbook, and rehearsal validation. No ontology, schema, canonical data, operational graph relationships, Supabase, snapshot/export pipeline, Visual Layers architecture, Passport derivation, Guided Investigation scenario *content*, navigation architecture, Risk calculations, or Timeline semantics changed. Full detail (the complete state manifest, the Demo Mode assessment, the browser rehearsal report, and the founder demo scripts/runbook themselves) is in the four new docs this sprint adds - this entry is a summary pointer, per this file's own established convention.

**Before coding:** verified `main` at `77413b5` (V1-GUIDE-1/PR #34 merged, matching the branch with zero divergence), `npm run build` baseline **1043/1043 tests**; read `guided-investigations/{nrs-01,nrs-02}.js` and `docs/GUIDED_INVESTIGATIONS.md` directly for the exact real object ids/relationships the demo scripts are authored from; reviewed `engine/state.js`, `engine/investigation-history.js`, `engine/investigation-presets.js`, `engine/guided-investigation-preferences.js`, `app.js`, and every panel/lens owning non-canonical local UI state.

**New docs:** `docs/DEMO_STATE_AUDIT.md` (Phase 1 state-and-reset manifest + Phase 8 Demo Mode assessment + Phase 9 rehearsal report), `docs/FOUNDER_DEMO_LONG.md` (31-beat, ~14-minute route: Executive Situation → NRS-01 → NRS-02 → Multiple Perspectives → Strategic Conclusion), `docs/FOUNDER_DEMO_SHORT.md` (9-beat, ~4-minute route built around a single guided investigation, deliberately NOT the long route sped up), `docs/FOUNDER_DEMO_RUNBOOK.md` (pre-demo prep, opening checklist, during-demo operating rules, post-demo reset, and a full failure-recovery matrix).

**Demo Reset, what shipped:** one control ("Reset Demo" / "Full Demo Reset") added to the existing Scenario Picker's header - not a new primary-navigation surface, per the brief. Two modes: transient (`resetDemo('transient')`) resets canonical `engine/state.js` fields (lens→Universe, no selection/focus, baseline time slice, zoom 0, unscoped, Full Enterprise Visual Layers), closes every modal/workspace this app owns (Scope Explorer, Visual Layers panel, Functional Radar workspace, Saved Views, Scenario Picker/completion, an active guided walkthrough), clears Search/Hover Preview/Risk Board recursion/the draggable Universe card offset, and wipes both navigation-history mechanisms (`focusTrail` and `engine/investigation-history.js`'s Back/Forward stacks) - all while **preserving** user-created Visual Layers presets, the saved default preset, the Functional Radar sync preference, and the guided-investigation "don't show again" dismissal. Full reset does everything transient reset does, requires a native `confirm()`, and additionally clears guided-investigation progress/completion/first-use invitation dismissal via the existing (previously unused-by-reset) `clearGuidedInvestigationPreferences()`. Neither mode ever calls `clearPersistedPresetData()` - user presets are never touched by Demo Reset in either mode, matching the explicit "do not delete user-created Visual Layers presets" requirement.

**Implementation, minimal by design:** every mutation routes through an existing public setter (`store.setState()`) or a small new panel/lens-local `reset()` method added only where local closure state had no public reset path yet - `panels/scope.js`, `panels/visual-layers.js`, `panels/functional-radar.js` (unconditional, unlike the pre-existing `close()`, which intentionally no-ops when already closed), `panels/universe-search.js`, `panels/hover-preview.js` (bypasses the 300ms hide-grace window for an instant, deterministic close), `lenses/risk-board.js` (`resetScope()`), and `engine/investigation-history.js` (`resetHistory()`, new). No second application state store; no page reload as the reset mechanism. All new `reset()` methods are idempotent (verified live: 5 repeated clicks, zero errors, zero drift).

**Two real defects found and fixed via Playwright, not guessed at:**
1. **Confirmed demo-blocking framework defect** (in scope per the brief's own exception - framework code, not scenario content): `panels/guided-investigation.js`'s document-level Escape listener called `isRunning(walkthrough)` with no null guard - `walkthrough` is `null` until the first guided scenario ever runs, so **any** Escape press closing an unrelated modal on a fresh boot threw an uncaught `TypeError`. Fixed with a one-line guard mirroring the identical check this same file's `render()` already uses.
2. **Demo-script accuracy issue** (content, not code): Universe Search for the bare fragment `SHP-NR-GOU-6101` resolves to a *different* real object (`nr04:exec:SHIPREL-NR-GOU-6101`, whose label also contains that fragment and wins the alphabetical tie-break) instead of the intended shipment. `engine/search.js` behaves exactly as designed - this was a demo-script authoring gap, fixed by documenting the unambiguous fragment `shipment:SHP-NR-GOU-6101` instead. Every other search fragment named in both demo scripts was verified against the real snapshot before publishing.

**Automated tests:** 23 new tests - `test/engine-investigation-history-live.test.mjs` (6, the live `goBack`/`goForward`/`resetHistory` binding, previously untested), `test/demo-reset-panels.test.mjs` (13, every new panel/lens `reset()` method plus idempotency), 4 new cases added to `test/panels-scenario-picker.test.mjs` (the Reset Demo/Full Demo Reset buttons and confirmation gating). `npm run build`: **1066/1066 tests** (1043 baseline + 23 new), check-syntax and verify-field-map both PASSED.

**Real browser verification (Playwright/Chromium):** 204/204 checks passed, 0 failures - Demo Reset from a deliberately messy state; Full Demo Reset's invitation-dismissal clearing (verified by reading the actual `localStorage` key before/after); user-created Visual Layers presets confirmed to survive **both** reset modes (seeded via `localStorage`, a real reload performed, both modes exercised); 5x repeated-click idempotency; reset while NRS-01 is actively running; **three consecutive full long-route rehearsals** (Risk Board → Universe Search → NRS-01 start-to-completion → NRS-02 start-to-completion, Demo Reset between each) with zero unexpected console errors and the third run exactly as reliable as the first; viewport coverage at 1440px/1280px/800px. Screenshots captured for the pristine post-reset state, the messy pre-reset state, and each rehearsal run's Act 1 beats.

**Demo Mode assessment (Phase 8):** no separate Demo Mode was built, by design - `docs/DEMO_STATE_AUDIT.md`'s own section explains why a deterministic reset plus documentation fully satisfies the brief without a parallel application mode, hidden flags, or branching render logic.

**Remaining-issue classification** (full detail in `docs/DEMO_STATE_AUDIT.md`):
- **Launch blocker:** none found this sprint.
- **Demo limitation:** the Risk Board's top-level `RB-CPP-HORIZON` card's own "View Contributing Objects" drill reaches an older, pre-NR04 curated recommendation thread, not the same NR04 governed chain NRS-01/NRS-02 trace from the governed commitment record - both are real, but they are two separate objects/chains for the same real-world customer (see `docs/FOUNDER_DEMO_LONG.md`'s own note and `docs/UNSUPPORTED_UI_FIELD_REPORT.md`). The demo scripts are written to be honest about this rather than paper over it.
- **Pilot limitation:** none newly identified this sprint beyond what prior sprints already logged.
- **V1.1:** Progressive Risk Board owner/next-action enrichment; exact focus-return accessibility after guided steps (carried forward from V1-GUIDE-1's own session log).
- **V2.0:** orchestration/action-taking on top of the governed context this demo proves - explicitly named as NOT current functionality in the demo's own closing narration (Act 5 / Beat L31, S9).
- **Not pulled into this sprint** (per the brief's explicit instruction): `resolveVisibilityForSlice()` t2/t3 gating gap; business-copy coverage beyond the 24 flagship objects; old curated demo objects coexisting with NR04 canonical objects (documented, not reconciled); narrow/mobile layout limitations.

**Deployment status:** no deployment pipeline exists for this repository (a static, zero-dependency prototype served via `scripts/serve.mjs`) - "deployment" here means the served app at the branch's current commit, which is what this sprint's own Playwright rehearsal ran against.
