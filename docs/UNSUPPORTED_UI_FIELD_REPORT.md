...
## New this sprint: V1-DEMO-1 Founder Demo Package

| Field / area | Classification | Note |
|---|---|---|
| `RB-CPP-HORIZON` (Risk Board cell) vs `nr04:commitment:CUST-HORIZON-CPP-2026-09` (NR04 canonical commitment) | **coexisting curated + canonical objects for the same real-world commitment** (clarified, not newly introduced - see the "carried over" entry above on the 9 curated flagship records) | Confirmed directly this sprint while authoring `docs/FOUNDER_DEMO_LONG.md`: these are two DISTINCT Universe graph nodes, both real, both about the same real Horizon LNG delivery, but not the same clickable object and not directly connected by an edge to each other. `RB-CPP-HORIZON`'s own one real outgoing edge (`has_recommendation`) reaches the OLDER pre-NR04 curated recommendation thread, not the NR04-governed chain NRS-01/NRS-02 trace from the canonical commitment. The founder demo scripts handle this with an explicit, narrated hand-off (Beat L4-L5) rather than treating them as interchangeable - a future data/derive session could reconcile the two (out of this sprint's scope per the brief's explicit "do not modify... operational graph relationships"). |
| Universe Search fragment ambiguity (`SHP-NR-GOU-6101`) | **derived_supported, documentation gap** (not a code gap) | `engine/search.js` behaves exactly as designed (label/id substring match, alphabetical tie-break) - the bare fragment matches two real objects' labels, and the non-intended one wins the tie-break. Not a bug; documented in `docs/FOUNDER_DEMO_RUNBOOK.md`'s Failure-Recovery Matrix and the demo scripts now use the unambiguous fragment `shipment:SHP-NR-GOU-6101`. |
| `panels/guided-investigation.js` Escape-key null-guard | **RESOLVED (V1-DEMO-1)** | See `docs/GUIDED_INVESTIGATIONS.md`'s Framework Review addendum and `docs/DEMO_STATE_AUDIT.md`'s Phase 9 report - a confirmed demo-blocking defect (Escape before any scenario ever ran crashed with an uncaught TypeError), fixed with a one-line null guard. |
